...
const buktiTransferManager = {
  ...
  /**
   * Save all bukti (for bulk operations like reset)
   */
  saveAll: (buktiData) => {
    try {
      saveBukti(buktiData);
      console.log(`✅ Saved ${buktiData.length} bukti transfer records`);
      return true;
    } catch (err) {
      console.error('❌ Error saving all bukti:', err.message);
      return false;
    }
  }
};
