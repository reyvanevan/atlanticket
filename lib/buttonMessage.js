...
async function sendInteractiveButton(conn, jid, options) {
    ...
    // Add optional header with title
    if (title) {
    ...
    if (image) {
        try {
            ...
            console.warn('⚠️ Failed to attach image:', error.message);
        }
    }
 ...
 * @param {string} [options.footer] - Optional footer text
 * @param {Array} options.sections - Array of sections [{ title, rows: [{ id, title, description }] }]
 * @param {Object} [options.quoted] - Optional quoted message
 * @returns {Promise} Send result
 */
