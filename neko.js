...
case 'setbot': {
  ...
  try {
    ...
    if (isOwner) {
      ...
      if (!filter) {
        // Show all bukti_transfer default
...
> Total : ${buktiSnapshot.size} data (latest 10)
┈ׅ──ׄ─꯭─꯭──────꯭ׄ──ׅ┈\n`;
        
        let no = 1;
        buktiSnapshot.forEach(doc => {
          const data = doc.data();
          riwayatText += `\n${no}. *${data.refID}*
...
\`.riwayat approved\` - Lihat approved
\`.riwayat [nomor_hp]\` - Lihat user tertentu`;
      ...
      if (filter === 'pending' || filter === 'approved' || filter === 'rejected') {
        ...
        let riwayatText = `📋 *RIWAYAT BUKTI TRANSFER - ${filter.toUpperCase()}*

> Total : ${buktiSnapshot.size} data
┈ׅ──ׄ─꯭─꯭──────꯭ׄ──ׅ┈\n`;
...
┈ׅ──ۄ─꯭─꯭──────꯭ׄ──ׅ┈`;
      ...
      // USER MODE - Lihat tiket mereka sendiri
...
> Total : ${ticketsSnapshot.size} tiket
┈ׅ──ׄ─꯭─꯭──────꯭ׄ──ׅ┈\n`;
